...
- 对完整圆形仪表，物理外圈是相机姿态基准，不只是展示框。明显斜拍时不能只验证单圈贴边；必须由不同半径的内外实体环和可见轴心共同证明同一透视姿态，再将表盘、轴心和指针端点一起校正到正视坐标后计算角度。透视校正后还要保持既有刻度零方向。信息不足时降级为仿射或未识别，不得输出伪精确结果。阶段报告中绿色为外圈、紫色为共同姿态下的内圈、蓝点为可见轴心，并记录外圈贴合率、内圈支持率、校正半径比例和实际采用的变换；
